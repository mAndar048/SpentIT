...
def parse_messages(messages):
    ...
    pattern = re.compile(r'Dear UPI user A/C [A-Z]\d+ debited by ([\d\.]+) on date (\d{2}[A-Za-z]{3}\d{2}) trf to (.+?) Refno \d+\. If not u\? call \d+\. -SBI')
...
def update_expense():
    ...
    conn = get_db_connection()
    conn.execute('UPDATE expenses SET date = ?, purpose = ?, amount = ? WHERE id = ?', (date, purpose, amount, expense_id))
    conn.commit()
    conn.close()
    return redirect('/user')
